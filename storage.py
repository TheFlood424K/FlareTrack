# storage.py
# JSON-based persistence layer with encryption support
...
class StorageManager:
    ...
    def __init__(self, data_dir: str = "data", use_encryption: bool = True):
        self.data_dir = data_dir
        self.patient_file = os.path.join(data_dir, "patient.json")
        self.logs_dir = os.path.join(data_dir, "logs")
        self.use_encryption = use_encryption
        self.encryption = DataEncryption() if use_encryption else None
        self._ensure_dirs()
    ...
    def _log_path(self, date_str: str) -> str:
        ext = ".enc" if self.use_encryption else ".json"
        return os.path.join(self.logs_dir, date_str + ext)
    ...
    def list_log_dates(self) -> List[str]:
        ...
        for filename in sorted(os.listdir(self.logs_dir)):
            if filename.endswith(".json") or filename.endswith(".enc"):
                dates.append(filename.rsplit(".", 1)[0])  # strip extension
        return dates
    ...
    def export_all_json(self, output_path: str) -> None:
        ...
        print(f"[Storage] Exported {len(all_data)} days to {output_path}.")
